...
- **objectGroup** (可选): 对象组名称（string）
- **includeCommandLineAPI** (可选): 是否包含命令行 API（boolean）
- **silent** (可选): 是否静默执行（boolean）
- **contextId** (可选): 执行上下文 ID（any）
- **returnByValue** (可选): 是否按值返回（boolean）
- **generatePreview** (可选): 是否生成预览（boolean）
- **userGesture** (可选): 是否模拟用户手势（boolean）
- **awaitPromise** (可选): 是否等待 Promise 完成（boolean）
- **throwOnSideEffect** (可选): 副作用时是否抛出错误（boolean）
- **disableBreaks** (可选): 是否禁用断点（boolean）
- **replMode** (可选): 是否 REPL 模式（boolean）
- **allowUnsafeEvalBlockedByCSP** (可选): 是否允许被 CSP 阻止的不安全 eval（boolean）
- **uniqueContextId** (可选): 唯一上下文 ID（string）
- **serializationOptions** (可选): 序列化选项（any）
...
- **elementName** (必需): 元素名称，必须是已通过 `find_element` 指令找到的元素（string）
- **text** (必需): 要输入的文本内容（string）
- **clear** (可选): 是否在输入前清空输入框（boolean，默认：false）
...
- **elementName** (可选): 元素名称，如果指定则先聚焦该元素（string）
- **action** (必需): 操作类型，可选值：
  - `"press"`: 按下并释放（完整按键事件）
  - `"type"`: 逐个字符输入
  - `"keydown"`: 按下按键
  - `"keyup"`: 释放按键
- **key** (必需): 按键名称或字符（string）
...
- **action** (必需): 鼠标操作类型，可选值：
  ...
  - `"right_mouseup"`: 右键释放
  - `"move_to"`: 移动到指定位置
- **elementName** (可选): 元素名称，如果指定则在该元素上执行操作（string）
- **x** (可选): 目标 X 坐标，如果指定了 `elementName` 则忽略此参数（number）
- **y** (可选): 目标 Y 坐标，如果指定了 `elementName` 则忽略此参数（number）
- **simulate** (可选): 鼠标轨迹模拟方式，可选值：
  - `"none"`: 不模拟，直接移动到目标位置
...
- **elementName** (必需): 元素名称，必须是已通过 `find_element` 指令找到的元素（string）
- **attribute** (可选): 要获取的属性名称（string）
- **usage** (可选): 使用方式，可选值：`"variable"` | `"data"` | `"none"`（string）
...
- **elementName** (必需): 元素名称，必须是已通过 `find_element` 指令找到的元素（string）
- **attribute** (必需): 要设置的属性名称（string）
- **value** (必需): 要设置的属性值（string）
...
- **format** (可选): 图片格式，可选值：`"png"` | `"jpeg"`（默认：`"png"`）
- **quality** (可选): 图片质量，范围 0-100，仅对 JPEG 格式有效（默认：100）（number）
- **fullPage** (可选): 是否截取整个页面（boolean，默认：false）
...
1. **元素引用顺序**：使用 `elementName` 的指令（如 `input`, `mouse`, `get_attribute`, `set_attribute`）必须在对应的 `find_element` 指令之后执行。

2. **延迟时间单位**：所有 `delay` 字段的单位都是**秒**，不是毫秒。

3. **重试机制**：`retry` 字段表示重试次数，`retry: 0` 表示不重试（但仍会执行一次），`retry: 3` 表示最多执行 4 次（初始 + 3次重试）。

4. **时间戳**：`created_at` 字段应使用 Unix 时间戳（毫秒）。

5. **指令ID唯一性**：每个指令的 `instructionID` 应该是唯一的。

6. **标签页ID**：确保 `tabId` 对应的标签页存在且已连接 CDP。
