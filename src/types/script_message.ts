 ...
 * - 消息类型包括：执行结果管理（get_results、clear_results、send_results_to_server）、指令执行（execute_instructions、pause_execution）、WebSocket 连接（connect_websocket、disconnect_websocket）、指令结果通知（instruction_result）
 ...
 * - type 字段为必需字段，必须是预定义的消息类型之一
 * - params 字段为可选字段，根据不同的消息类型，params 的结构会有所不同
 * - 消息类型包括：生命周期（content_script_loaded、setCallbacks）、标签页管理（get_tabs、send_tabs）、节点配置（get_node_profile、update_node_profile）、指令管理（add_instructions、execute_instructions、pause_execution、stop_execution）、执行状态（get_executor_status）、结果管理（get_results、clear_results、send_results_to_server）、WebSocket 连接（connect_websocket、disconnect_websocket、test_websocket）
...
export interface BackgroundScriptMessageType {
    type: 'content_script_loaded' | 'get_tabs' | 'send_tabs' | 'get_node_profile' | 'update_node_profile' | 'add_instructions' | 'execute_instructions' | 'pause_execution' | 'stop_execution' | 'get_executor_status' | 'get_results' | 'clear_results' | 'connect_websocket' | 'disconnect_websocket' | 'test_websocket' | 'send_results_to_server' | 'setCallbacks';
    params?: any;
}
